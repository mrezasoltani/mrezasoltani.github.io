...
* **Random Variables.** A random variable (r.v.) \\(X: \Omega\rightarrow \mathbb{R}\\) is a function from the sample space to the real line.
  ...
  - **Multivariate Random variables.** A cdf of a real vlaued random vector \\(\mathbf{X} \in \\mathbb{R}^{p}\\) (aka multivariate real-valued random variable) is given by:
    ...
    - For the continous case, the cdf can be expressed also as the integral of the pdf \\(p(\mathbf{x})\\) as follows:
      \\[Pr(\mathbf{x}) = \int _{\mathbf{x} \in \Mathbb{R}^pn}p(\mathbf{x})d\mathbf{x} = \int _{-infty}^{x_p} \int _{-infty}^{x _{p-1}\ldots \int _{-infty}^{x_1}dx_px_2\ldots x_p p(x_1, x_2, \ldots, x_p)\\]
