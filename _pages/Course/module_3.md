...
* Generally, \\(K^{th}\\) moment can be defined in different ways (we only state the formulas for the continuous case. The discrete case is has the same formula by replacing "integral" with "sum"):
