...
* **Conditional Probability.** The conditional probability of happening an event \\(A\\) given that another event \\(B\\) has occurred is given by \\(P(A\|B) = \frac{P(A, B)}{P(B)}, ~ P(B)\neq 0\\).
  - **Product Rule.** From the above relation, we can wrtie \\(p(A, B) = P(A\|B)\cdot P(p(B)\\). This is called the product rule.
...
* **Quantiles.**
  ...
  - The \\(q^{th}\\) quantile of the cdf of a random variable \\(X\\) denoted by \\(x_q\\) and is defined by \\(P^{-1}(q)\\) where \\( Pr(X \leq x_qq) = q \\).
