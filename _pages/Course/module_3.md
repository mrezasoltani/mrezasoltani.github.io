...
* **Random Variables.** A random variable (r.v.) \\(X: \Omega\rightarrow \mathbb{R}\\) is a function from the sample space to the real line.
  - **Discrete random variables.** If the sample space \\(\Omega\\) over which a random variable \\(X\\) is defined is finite or countably infinite, then \\(X\\) is called a discrete random variable. Any possible realization of \\X\\) is an event, hence, the probability of the event that \\(X\\) has value \\(x\\) is given by \\(P(X=x)\\) (Most of the time for simplicity of notation, we write \\(P(x)\\)).
    ...
    - **Cumulative distribution function (cdf).** We define a cdf of a r.v. as \\(Pr(x) = Pr_X(x) =  Pr(X\leq x) = F(x) = \sum _{\alpha \in \Omega, \alpha\leq x} P(\alpha)\\). From this definition, we immediately see that \\(Pr(\alpha \leq X \leq \beta) = Pr(\beta) - Pr(\alpha)\\).
  ...
  - **Multivariate Random variables.** A cdf of a real vlaued random vector \\(\mathbf{X} \in \\mathbb{R}^{p}\\) (aka multivariate real-valued random variable) is given by:
    \\[Pr(\mathbf{x}) = Pr_{\mathbf{X}}(\mathbf{x}) = F__{\mathbf{X}}(x) = Pr(X_1\leq x_1, X_2\leq x_2, \ldots, X_n\leq x_n)\\]
    - For the continous case, the cdf can be expressed also as the integral of the pdf \\(p(\mathbf{x})\\) as follows:
      \\[Pr(\mathbf{x}) = \int _{\mathbf{x} \in \Omega}p(\mathbf{x})d\mathbf{x}\\]
