...
* Let's recall the goal of regression. We are given observation data, including a set of \\(n\\) i.i.d. input-output pairs (_training data_) \\(\mathcal{D}=\\)             
...
<details>
  <summary>Proof</summary>
    ...
    & \hspace{-3cm} \text{Where the minimum in the last inequality is achieved if we choose} \\(\hat{f}(\mathbf{x})=\mathbb{E}\big{(}Y\|\mathbf{X}=\mathbf{x}\big{)}\\).
        \end{aligned}
    \end{equation}
