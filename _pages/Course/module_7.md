...
\\[f^{\*} = \text{argmin}_{\hat{f}}\mathbb{E} \big{(}Y - \hat{f}(\mathbf{X})\big{)}^2 = \text{argmin} _{\hat{f}}\mathbb{E} _{\mathcal{D}_n}_\mathbb{E} _{\mathbf{X}, Y}\Big{(}\big{(}Y - \hat{f}(\mathbf{X})\big{)}^2\|\mathcal{D}_n\Big{)}\\]
